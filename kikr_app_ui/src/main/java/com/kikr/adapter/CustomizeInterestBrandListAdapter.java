...
public class CustomizeInterestBrandListAdapter extends BaseAdapter{
    ...
    public CustomizeInterestBrandListAdapter(FragmentActivity mContext,List<InterestSection> stores, CustomizeFeedActivity customizeFeedActivity) {
                ...
                if(!fromProfile) {
                    if (!TextUtils.isEmpty(brands.get(position).getIs_followed())&&customizeFeedActivity.checkInternet()) {
                        if(brands.get(position).getIs_followed().equals("yes")){
                            ...
                            customizeFeedActivity.deleteBrand(getItem(position).getId(),viewHolder.checkImageView.getRootView());
                            notifyDataSetChanged();
                        }else{
                            brands.get(position).setIs_followed("yes");
                            customizeFeedActivity.addBrand(getItem(position).getId(),viewHolder.checkImageView.getRootView());
                            notifyDataSetChanged();
                        }
                    }
                }else {
