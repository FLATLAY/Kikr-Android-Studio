...
//		view=convertView;
	...
	public View getView() {
		...
		for (int i = 0; i < data.size(); i++) {
			View convertView = (LinearLayout) mInflater.inflate(R.layout.adapter_discover_small,null);
			...
			activity_product_list_product_price1.setText(" $"+CommonUtility.getFormatedNum(product.getRetailprice()));
			...
			activity_product_list_product_name.setText(TextUtils.isEmpty(product.getBrand())?product.getProductname():product.getBrand());
		...
		if(data.size()>0){
			...
			LayoutParams layoutParams2 =new LinearLayout.LayoutParams(CommonUtility.getDeviceWidth(mContext)/2, LinearLayout.LayoutParams.MATCH_PARENT);
			...
			convertView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					addFragment(new FragmentProductBasedOnInspiration("inspiration", "", inspiration.getInspiration_id()));
				}
			});
		}
		return ll;
